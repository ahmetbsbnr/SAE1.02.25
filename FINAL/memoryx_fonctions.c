...
void initialiser_plateau(Jeu *jeu) {
    ...
    /* Selon le sujet Section 5 : P contient les positions des cartes restantes */
    /* Après génération, toutes les positions sont encore en jeu */
    for (i = 0; i < taille_totale; i++) {
        jeu->P[i] = i;
    }
    /* Note: P sera mis à jour quand des paires sont trouvées (R diminue) */
}
...
void gestion_joker(Jeu *jeu, int positionJoker) {
    ...
    /* Car le Joker a bougé et la carte à l'autre position aussi */
